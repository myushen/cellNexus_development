...
tar_script({
  ...
  sample_heuristics = function(col_data){
    ...
    col_data |>
      ...
      # Some cell ids are dbl
      mutate(cell_ = as.factor(cell_))
  ...
  select_sample_columns = function(metadata){
    ...
    sample_column_to_preserve =
      metadata |>
      slice_sample(n = 500, by = donor_id) |>
      distinct(sample_, .keep_all = TRUE) |>
      # Drop some clearly cell-wise columns
      select(-any_of(c("observation_joinid", "cell_")), -contains("cell_type"), -contains("X_pca"), -contains("X_umap")) |> 
      #tidybulk::pivot_sample(.sample = sample_) |>
      colnames()
  ...
  list(
    ...
    tar_target(
      files_dataset_id,
      # cellNexus 2024 uses schema_version 5.1.0
      datasets(my_db) |>
        left_join(
...
# Sample metadata
saved <- tar_read(metadata_dataset_id_common_sample_columns, store = glue("{result_directory}/_targets")) |>
  write_parquet("/vast/projects/cellxgene_curated/metadata_cellxgenedp_Apr_2024/sample_metadata.parquet", compression = "zstd")

# Sample to cell link
saved <- tar_read(metadata_dataset_id_cell_to_sample_mapping, store = glue("{result_directory}/_targets")) |>
...
tar_read(dataset_cell_dict, store = glue("{result_directory}/_targets"))|>bind_rows() |>
  write_parquet("/vast/scratch/users/shen.m/test_cellnexus_reproducibility/dataset_cell_dict.parquet",
                compression = "zstd")
